...
public class RedisConfig {
    ...
    public ObjectMapper objectMapper() {
        ...
        // 添加UserPO.UserPOMixin，用于在序列化和反序列化时忽略UserPO中的password属性
        mapper.addMixIn(UserPO.class, UserPO.UserPOMixin.class);

        // 注册 JavaTimeModule 模块，以支持 Java 8 日期时间 API 的序列化和反序列化（LocalDateTime）
        mapper.registerModule(new JavaTimeModule());

        // 禁用在反序列化时遇到未知属性时抛出异常，允许在反序列化时忽略未知属性
